...
void Widget::paintEvent(QPaintEvent *event)
{
    ...
    GridSize = std::min(size().width(), size().height()) / (Child.Size + 2.5); //棋盘格子大小
    int theme = 0; //
    ...
    int Width = (Child.Size - 1) * GridSize; //棋盘宽度
    int Grid = GridSize * 2;
    ...
    if (GridSize > 6) {
        ...
        if (Radius >= 0) {
                ...
                for (int i = 0, x = BOARD_LEFT; i < Child.Size; i++, x += GridSize) {
                    ...
                    if ((View & VIEW_SCORE) && label != BOTH_AREA) {
                        ...
                        if (label == BLACK_AREA || label == WHITE_DEAD) {
                            if (theme) 
								painter.setBrush(QBrush(Child.Status[k].Color));
                            else 
								painter.setBrush(QBrush(QColor(0, 0, 0), Qt::BDiagPattern));
                        }
                        else {
                            if 
								(theme) painter.setBrush(QBrush(Child.Status[k].Color));
                            else 
            ...
            if (Cursor[0].x() >=0 && Cursor[0].y() >= 0) {
				QColor color = Child.Record2.Index < 0 ? (Child.State.Turn == BLACK ? Qt::black : QColor(240, 240, 240)) : Qt::red;
...
void Window::trigerMenu(QAction * act)
{
	...
	{
		if (Child->Child.Forward())
			Child->ShowTable();
	}
	else if (act->text() == u8"形势判断")
	{
		Child->View ^= Child->VIEW_SCORE;
		if (Child->View & Child->VIEW_SCORE) 
			Child->Child.Rand(2000);
		repaint();
	}
	...
	{
		Child->Child.BOARD_FILE = "001.SGF";
		if (Child->Child.Write(Child->Child.BOARD_FILE)) {
			Child->Child.BOARD_FILE.clear();
			SetTitle("SAVE");
			return;
		}
	}
	else if (act->text() == u8"PASS")
	{
		...
		QCoreApplication::sendEvent(Child, &pKey1);
	}
